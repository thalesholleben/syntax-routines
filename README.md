...
> The panel, the CLI output and the agent skill are in Brazilian Portuguese, because that is
> the audience the product was built for. The code, the tests and this documentation are in
> English; a Portuguese version of this page is [README.pt-BR.md](README.pt-BR.md).
...
Demo data, generated by `node scripts/screenshots.mjs` on a disposable installation; no real
routine appears here.

### Routines

![Routine list with four cards: two scripts, one Claude Code routine and one Codex routine, with next run and history](docs/assets/screenshots/rotinas.png)

### Editing an agent routine

![Edit dialog of a Claude Code routine: executor, model, effort, days, time and prompt](docs/assets/screenshots/modal-agente.png)

### Script routine on an interval

![Edit dialog of a script routine every 15 minutes, with the command and the directory](docs/assets/screenshots/modal-script.png)

### Settings

![Settings: root folder, binaries, parallel runs, notification e-mail and scheduler status](docs/assets/screenshots/ajustes.png)
...
| `node scripts/screenshots.mjs` | regenerates the README images on a disposable installation |
